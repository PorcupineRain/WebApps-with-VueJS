Vue.createApp({
  ...
  computed: {
    columnNames() {
      ...
      const bookKeys = Object.keys(this.books[0]);
      return bookKeys.filter((key) => columnNames.includes(key));
    },
    filteredBooks() {
  ...
  methods: {
    calculatedSalePrice(price) {
      const priceValue = price.replaceAll("$", "");
      const newPrice = priceValue * 0.8;
